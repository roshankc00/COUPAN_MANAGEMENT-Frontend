...
const CouponCard: React.FC<Props> = ({ coupon }) => {
  ...
  const handleDeal = async () => {
    if (isLogedInStatus) {
      ...
      router.push(
        `/alert?alert=${coupon?.store?.affiliateLink.link}&id=${coupon?.store?.affiliateLink?.id}&cashbackAmountPer=${coupon?.store?.affiliateLink?.cashbackAmountPer}`
      );
    }
  };
      ...
      <div className="flex gap-5 items-center justify-between py-4 sm:py-0">
          ...
          <div>
            ...
            <div className="flex gap-3 mt-2 items-center">
              <div
                ...
                onClick={() => router.push(`/store/${coupon?.store?.slug}`)}
              >
        ...
        <div className="">
          {coupon?.isDeal ? (
            ...
            >
              <span className="z-10 line-clamp-1">
                ........{coupon?.code?.slice(0, 6)}
              </span>
