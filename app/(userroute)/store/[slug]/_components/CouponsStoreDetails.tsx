...
import { UseGetCouponInfoWithSlug } from "@/hooks/react-query/coupons/get-coupon-withslug";
type Props = {
  storeSlug: string;
};

const CouponStoreDetails: React.FC<Props> = ({ storeSlug }) => {
  const params = useSearchParams();
  const [isCoppied, setisCoppied] = useState(false);
  const couponSlug = params.get("coupon");
  const [open, setopen] = useState(couponSlug ? true : false);
  ...
  const {
    ...
    isLoading,
    refetch,
  } = UseGetAllCouponsOfStore(storeSlug, paginationProps.currentPage, 30);
  ...
  } = UseGetStoreInfoWithSlug(storeSlug);
  ...
  const { data: itemExist, isLoading: existLoading } =
    UseItemExistInFollowerlist(storeDetails?.id);
  ...
  const {
    data: modelCouponData,
    isFetching: modelCouponFetching,
    isLoading: modelCouponLoading,
    refetch: modelCouponRefetch,
  } = UseGetCouponInfoWithSlug(couponSlug);
  ...
  const handleNavigate = () => {
    if (isLogedInStatus) {
      ...
      router.push(
        `/alert?alert=${storeDetails?.affiliateLink.link}&id=${storeDetails?.affiliateLink?.id}&cashbackAmountPer=${storeDetails?.affiliateLink?.cashbackAmountPer}`
      );
    }
  };
  ...
  return (
    <>
      {!modelCouponFetching && !modelCouponLoading && (
        <div>
          <div>
            <Dialog open={open} onOpenChange={setopen}>
              <DialogContent className="w-[30%]">
                <div className="flex flex-col  items-center justify-center mb-10">
                  ...
                  <h1 className="text-xl -my-5">{modelCouponData?.tag}</h1>
                </div>
                <div>
                  ...
                  <div className="flex justify-center gap-2 my-1">
                    ...
                    <button
                      ...
                      onClick={() => copyToClipboard(modelCouponData?.key!)}
                    >
                ...
                <Separator />
                <p className="text-center">{modelCouponData?.description}</p>
              </DialogContent>
            </Dialog>
          </div>
